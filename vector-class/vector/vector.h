...
typedef struct {
  ...
  int vecSize; // vector size should be 4 at first. than grow it. 
  int elemSize; // specifies the number of bytes that a single element of the vector should take up
  VectorFreeFunction free;
  int sizeNow; // what size is the vector right now;
  void* vec;
